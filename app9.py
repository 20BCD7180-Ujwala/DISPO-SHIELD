...
Original file is located at
    https://colab.research.google.com/drive/1pMdbwIJ9G0DrEZOp40PtbWvErOrZ-6BH
"""
...
rad=st.sidebar.radio("Navigation",["Home","Spam/Ham Detection","Sentiment Analysis","Hate and Offensive Content Detection","Sarcasm Detection"])

#Home Page
if rad=="Home":
    st.title("DISPO SHIELD - A Multi-faceted IntelliText Analyser")
    st.image("Home Page.jpg")
    st.text(" ")
    st.text("The Following Text Analysis Options Are Available->")
    st.text(" ")
    st.text("1. Spam/Ham Detection")
    st.text("2. Sentiment Analysis")
    st.text("3. Hate and Offensive Content Detection")
    st.text("4. Sarcasm Detection")
...
model2_1=LogisticRegression(random_state=42)
model2_2=MultinomialNB()
model2=VotingClassifier(estimators=[('logistic_regression', model2_1),('naive_bayes', model2_2)], voting='soft')
...
model4_1=RandomForestClassifier(n_estimators=100, random_state=42)
model4_2=decision_tree_model = DecisionTreeClassifier(random_state=42)
model4=VotingClassifier(estimators=[('decision_tree', model4_2),('random_forest', model4_1)], voting='soft')
